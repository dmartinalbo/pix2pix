...
for epoch = 1, opt.niter do
  ...
  for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
    ...
    xlua.progress(i, math.min(data:size(), opt.ntrain))

    -- Compute EPOCH (not batch) errors
    train_loss_l1_epoch = train_loss_l1_epoch + errL1 * opt.batchSize
    ...
    --             data_tm:time().real / opt.batchSize,
    --             errG and errG or -1, 
    --             errD and errD or -1, 
    --             errL1 and errL1 or -1)
    --           )
    -- end
